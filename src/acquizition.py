...
class Acquirer:
    ...
    def do_solve_and_correct(self, target_ra_j2000_hours: float, target_dec_j2000_degs: float):
        op = function_name()
        ...
        if not self.unit.solver.solve_and_correct(exposure_settings=settings,
                                                  target=target,
    ...
    def do_acquire(self, target_ra_j2000_hours: float, target_dec_j2000_degs: float):
        ...
        logger.info(f"{op}: phase #1, stage at Sky position")
        #
        ...
        #
        logger.info(f"{op}: phase #2, stage at Spec position")
        ...
        success = self.unit.solver.solve_and_correct(exposure_settings=phase2_settings,
                                                     ...
                                                     parent_activity=UnitActivities.Acquiring,
                                                     max_tries=tries)
        logger.info("{op}: phase #2 (stage at Spec) " + 'succeeded' if success else 'failed')
        if success:
            self.unit.reference_image = self.unit.camera.image
        self.unit.end_activity(UnitActivities.Acquiring)

        logger.info("{op}: >>> We could start guiding now, but not yet! <<<")
    ...
    def start_one_solve_and_correct(self, ra_j2000_hours: float, dec_j2000_degs: float):
        Thread(name='solve-and-correct', target=self.do_solve_and_correct, args=[ra_j2000_hours, dec_j2000_degs]).start()
